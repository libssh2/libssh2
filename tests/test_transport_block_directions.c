...
/* fake a half-sent packet. returns the pointer to pass to
   ssh2_transport_send() so it matches p->odata. */
                                  ...
                                  const char *label)
{
    ...
    if(rc != LIBSSH2_ERROR_EAGAIN) {
        fprintf(stderr,
                "%s: ssh2_transport_send returned %d, expected EAGAIN\n",
                label, rc);
        return 1;
    }
    ...
    if(!(bd & LIBSSH2_SESSION_BLOCK_OUTBOUND)) {
        fprintf(stderr,
                ...
                label, (unsigned int)bd,
                (unsigned int)LIBSSH2_SESSION_BLOCK_OUTBOUND);
        return 1;
    }
    return 0;
}
...
static int test_addr_mismatch(LIBSSH2_SESSION *session)
{
    ...
    rc = ssh2_transport_send(session, other, sizeof(other) - 1, NULL, 0);
...
/* callback_set2 wants libssh2_cb_generic *; the concrete type is a
   different function pointer. Both -Wcast-function-type variants flag it,
   same trick as src/session.c. */
#ifdef __clang__
...
#endif
#endif
static libssh2_cb_generic *swap_send_cb(LIBSSH2_SESSION *session,
                                        libssh2_cb_generic *cb)
{
    return libssh2_session_callback_set2(session, LIBSSH2_CALLBACK_SEND, cb);
}

static libssh2_cb_generic *stub_send_as_generic(void)
{
    return (libssh2_cb_generic *)stub_send;
}
#ifdef __clang__
...
static int test_send_eagain(LIBSSH2_SESSION *session)
{
    ...
    prev_send = swap_send_cb(session, stub_send_as_generic());

    data = arm_pending_packet(session, queued, len, 0);
    session->socket_block_directions = 0;

    rc = ssh2_transport_send(session, data, len, NULL, 0);

    disarm_pending(session);
    swap_send_cb(session, prev_send);
