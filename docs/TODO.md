...
* remove the existing `libssh2_knownhost_check()` function and rename
  `libssh2_knownhost_checkp()` to become the new `libssh2_knownhost_check`
  instead.
...
slow. (The original API had the `libssh2_poll_channel_read()` and
`libssh2_poll()` to somewhat overcome this hurdle, but they too have pretty
